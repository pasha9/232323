...
void C_KeyBind::draw( ) {
	...
	if ( item_region.contains_point( mouse ) ) {
		render::get( ).rect_filled( n_area.x, n_area.y, n_area.w, 20, Color { 65, 65, 65, 255 } );
	}
	else
		render::get( ).rect_filled( n_area.x, n_area.y, n_area.w, 20, Color { 55, 55, 55, 255 } );
	...
	render::get( ).text( fonts[ TAHOMA12 ], n_area.x + 10, n_area.y + 10, text_color, HFONT_CENTERED_Y, to_render.c_str( ) );
}
