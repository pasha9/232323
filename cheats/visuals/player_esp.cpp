...
void playeresp::draw_ammobar( player_t* m_entity , Box box ) {
	...
	if ( weapon->m_iClip1() != weapon->get_csweapon_info()->max_clip )
		render::get().text( fonts [ TAHOMA12 ] ,
...
void playeresp::draw_name( player_t* m_entity , Box box ) {
	...
	if ( g_csgo.m_engine()->GetPlayerInfo( m_entity->EntIndex() , &player_info ) ) {
		...
		render::get().text( fonts [ TAHOMA12 ] , box.x + ( box.w / 2 ) , box.y - 13 , color , HFONT_CENTERED_X , name.c_str() );
	}
}
...
void playeresp::draw_weapon( player_t* m_entity, Box box ) {
	...
	render::get( ).text( fonts[ TAHOMA12 ], box.x + ( box.w / 2 ), _y, color, HFONT_CENTERED_X, weapon->get_name( ) );
}
	...
	if ( g_cfg.player.flags[ FLAGS_VULNERABLE ].enabled ) {
		...
		if ( player.m_e && player.m_resolver ) {
			if ( player.m_resolver->m_mode == resolver_modes::lby || player.m_resolver->m_mode == resolver_modes::lby_update )
				render::get( ).text( fonts[ ESPFLAG ], box.x + ( box.w / 2 ), box.y - 22, Color::Red, HFONT_CENTERED_X, "VULNERABLE" );
		}
	}
